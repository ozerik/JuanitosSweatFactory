...
byte targetBrightness[12];   // holds brightness values
bool potOneButton;           // button under top left pot
...
void setup() {
  ...
  pinMode(PIN_PD3, INPUT);  // topRowPot 4
  pinMode(PIN_PE7, INPUT);  // top row ACTUAL pot 1, the CV and pot are separate
  pinMode(PIN_PD7, INPUT);  // top row ACTUAL pot 4,

  pinMode(9, INPUT_PULLUP);   // clicky button under pot 1
  pinMode(10, INPUT_PULLUP);  // clicky button under pot 4
  pinMode(5, INPUT_PULLUP);   // mechanical keyboard switch for SHIFT
  ...
  analogReadResolution(12);           // the AVR128DB has 12 bit analog read resolution! May as well use it
...
void setupClock() {
  ...
  // okay, setting up TCA1 as prescaler number one:
  TCA1.SINGLE.CTRLA = TCA_SINGLE_CLKSEL_DIV256_gc
                      | TCA_SINGLE_ENABLE_bm;

  // then say "HEY! TCB0!!! Use TCA1 as the clock source!" Also, divide by 2
  TCB0.CTRLA = TCB_CLKSEL_TCA1_gc;

  // okay, turning on "Periodic Interrupt" mode. Timer counts to CCMP, fires the interrupt, resets
  TCB0.CTRLB = TCB_CNTMODE_INT_gc;

  // initial tempo = 120BPM, you know, for house music
  TCB0.CCMP = 976;  // CCMP stands for Compare/Capture Match Point
